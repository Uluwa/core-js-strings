 ...
 */
function removeLeadingWhitespaces(st) {
  return st.trimStart();
}
 ...
 */
function removeTrailingWhitespaces(abs) {
  return abs.trimEnd();
}
...
function sumOfCodes(str) {
  ...
  //   if (sum.charCodeAt(i) > 0) {
  //     result.push(sum.charCodeAt(i));
  //   }
  // }
  // return result.reduce((acc, curr) => acc + curr, 0);
  if (!str) {
    return 0; // Return 0 for empty or undefined strings
  }
...
function countVowels(str) {
  return str.split('').filter((words) => 'aeiouyAEIOUY'.includes(words)).length;
}
...
function getStringFromTemplate(firstName, lastName) {
  const a = 'Hello, ';
  return a.concat(firstName, ' ', lastName, '!');
}
